...
@section('title', 'العمالة - قطاف القصيم')
...
<div class="workers-shell">
    <div class="workers-header">
        ...
        <div class="workers-actions">
            <a href="{{ route('farmer.requests') }}" class="secondary-action">طلبات العمالة</a>
            <a href="{{ route('farmer.request.create') }}" class="primary-action">+ طلب عمالة جديد</a>
        </div>
    </div>
            ...
            @foreach ($workers as $assignment)
                ...
                <article class="worker-card">
                    <div class="worker-card-header">
                        ...
                        <div>
                            <h2>{{ $workerName }}</h2>
                            <p class="worker-specialization">{{ $worker?->specialization ?: 'عامل زراعي' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="worker-details">
                        ...
                        <div class="worker-detail">
                            ...
                            <span class="worker-detail-value">{{ $worker?->years_experience ?? 0 }} سنة</span>
                        </div>
                        <div class="worker-detail">
                            <span class="worker-detail-label">الأجر المتفق</span>
                            <span class="worker-detail-value">{{ $assignment->agreed_wage ? number_format($assignment->agreed_wage, 2) . ' ر.س' : 'حسب الطلب' }}</span>
                        </div>
                    </div>

                    <div class="worker-request">
                        <span class="text-muted">مرتبط بطلب:</span>
                        <a href="{{ route('farmer.request.show', $assignment->laborRequest->id) }}">
                            {{ $assignment->laborRequest->service_type }} #{{ $assignment->laborRequest->id }}
                        </a>
                    </div>
                </article>
            @endforeach
    ...
    @else
        <div class="empty-workers">
            ...
            <a href="{{ route('farmer.request.create') }}" class="workers-actions primary-action d-inline-flex mt-3">إنشاء طلب عمالة</a>
        </div>
    @endif
</div>
@endsection
